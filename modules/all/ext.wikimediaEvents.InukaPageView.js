...
( function () {
	...
	mw.requestIdleCallback( function () {
		var sectionSelector = 'h2.section-heading',
			openedSections = {},
			msPaused = 0,
			sectionCount,
			pageNamespace,
		...
		function logEvent() {
			...
			if (performance.timing.domInteractive) {
				start = performance.timing.domInteractive;
				totalTime = now - start;
			} else {
				// DOM not yet interactive: we consider the load time to be now
				// and the time spent on page to be 0.
				// If a duplicate event is logged later for the same page view,
				// those values will be more reliable.
				start = now;
				totalTime = 0;
			}
